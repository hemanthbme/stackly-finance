...
DECLARE
  ...
  v_total numeric;
BEGIN
  -- WIPE: only households created by the demo user
  ...
  -- ACCOUNTS + 26 weekly snapshots (Sundays)
  FOR v_acc IN
    SELECT * FROM (VALUES
  ...
  WHILE v_d <= CURRENT_DATE LOOP
    ...
    IF v_weekend THEN
      v_n := 2 + floor(random() * 2)::int;   -- 2..3
    ELSE
      v_n := floor(random() * 2.2)::int;      -- 0..2
    END IF;
  ...
  -- ensure every one of the 7 months has at least one entry
  FOR v_i IN 0..6 LOOP
  ...
  -- BUDGET: app reads combined + daily; monthly = daily_limit * 31
  ...
  INSERT INTO public.household_goals (household_id, label, type, target, created_by) VALUES
    ...
    (v_hh, 'Pay down total debt below $400k', 'debt', 400000, v_uid);
