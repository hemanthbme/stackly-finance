...
AS $$
  SELECT COALESCE(
    lower(auth.jwt() ->> 'email') = ANY (ARRAY['hmdbme@gmail.com','hemanth.bme@gmail.com']),
    false
  );
$$;

-- Expanded admin summary with engagement metrics (no financial figures)
...
BEGIN
  ...
  SELECT
    ...
    agg.spending_total,
    agg.last_entry_at,
    -- Activity score 0..100: recency of last activity (up to 60) + 30d entry volume (up to 40)
    LEAST(
      100,
      (
        CASE
